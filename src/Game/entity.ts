...
import { isAttacker, isDestructible, isPlayerActor } from './mixins';
...
export class Entity extends Glyph {
  ...
  tryMove(
    ...
    z: number,
    map?: Map,
    setCallback?: (x: number, y: number, z: number) => void
  ) {
    const moved = () => {
      this.setPosition(x, y, z);
      setCallback && setCallback(x, y, z);
      this.dispatchMoved(x, y, z);
    };
